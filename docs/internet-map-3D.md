...
`internet-map-3d` is the standalone 3D emulator topology frontend. This document currently focuses on the two emulator topology pages:

- `/uploadMap`: file-based emulator topology page backed by `emulatorTopology3D.vue`.
- `/map`: live emulator topology page backed by `liveEmulatorTopology3D.vue` and Docker API data through `emulator-service`.

The Docker Compose service name is `internet-map-3d`. The default published port is `8090:80`.
...
flowchart TB
  subgraph Frontend["internet-map-3d frontend"]
    Offline["/uploadMap<br/>file-based compose topology"]
    Live["/map<br/>live Docker topology"]
  end
...
| --- | --- | --- |
| `/uploadMap` | `view/map/emulatorTopology3D/emulatorTopology3D.vue` | 3D emulator topology page using local Docker Compose data. |
| `/map` | `view/map/liveEmulatorTopology3D/liveEmulatorTopology3D.vue` | 3D emulator topology page using live Docker API data. |
...
%%{init: {"flowchart": {"useMaxWidth": true, "htmlLabels": true}} }%%
flowchart TB
  Route["Route<br/><code>/uploadMap</code>"]
...
flowchart LR
  ...
  subgraph RenderFlow["Cesium render and animation flow"]
    MapGlobe["components/Map3DGlobe/index.vue<br/><code>render()</code><br/><code>animatePacketHop()</code><br/><code>animatePacketPath()</code><br/><code>clearPacketAnimations()</code>"]
  ...
  RenderGraph --> GraphBuilder --> DisplayGraph --> MapGlobe --> Scene
  ...
  ReplayFns --> AnimateFns --> MapGlobe --> Scene
```
...
flowchart LR
  ...
  subgraph RenderFlow["Cesium render and animation flow"]
    MapGlobe["components/Map3DGlobe/index.vue<br/><code>render()</code><br/><code>animatePacketHop()</code><br/><code>flashNodes()</code>"]
    Scene["shared/services/cesiumScene.ts<br/><code>createMap3DScene()</code><br/><code>renderGraph()</code><br/><code>animatePacketHop()</code><br/><code>updatePacketHops()</code>"]
  end

  LoadTopology --> ApiMap --> Request --> EmulatorApi --> SetTopology --> RenderGraph
  LoadFilter --> TrafficSvc --> Observer
  RenderGraph --> GraphBuilder --> MapGlobe --> Scene
  RenderGraph --> Search
...
The frontend reads environment files from `internet-map-3d/frontend/env`.
...
- `internet-map-3d/Dockerfile`
- `internet-map-3d/nginx.conf`
- `internet-map-3d/entrypoint.sh`
...
- [internet-map-3d-testing.md](./test/internet-map-3d-testing.md)
