# internet-map-3d 测试覆盖文档

本文档描述 `internet-map-3d/frontend` 当前测试用例覆盖的模块、页面和测试函数。

> 当前 Windows 工作区中的目录可能仍显示为 `internet-map-3D`；CI 和文档按目标目录名 `internet-map-3d` 描述。
...
flowchart TB
  ...
  Map3DGraph["view/map3dGraph.test.ts"]
  ...
  Map3DGraph --> Map3DScope["3D graph helpers"]
```
...
| `tests/unit/view/map3dGraph.test.ts` | 3D map graph helpers | `builds stable undirected edge keys`<br/>`identifies routers and border routers as transit routers`<br/>`adds AS highlight nodes near visible satellite-connected routers` |
...
flowchart TB
  ...
  MapPages --> Map3DScope["3D map pages"]
```
...
| `tests/e2e/map-pages.spec.ts` | IXMap3D page | `IXMap3D page loads toolbar and toggles settings` |
| `tests/e2e/map-pages.spec.ts` | Upload Map page | `UploadMap page exposes the upload entry for 3D graph rendering` |
